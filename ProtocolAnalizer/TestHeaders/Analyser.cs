...
namespace ProtocolAnalizer
{
    public class Analyser
    {
        ...
        public static IEnumerable<ServerInfo> FullAnalyse(IEnumerable<string> e)
        {
            ...
            foreach (var item in e)
            {
                if (serverList.Count(w => w.Name == NormaliseName(item)) == 0)
                {
                    serverList.Add(new ServerInfo(NormaliseName(item), item));
                }
                else
                {
                    serverList.First(w=>w.Name==NormaliseName(item)).Add(GetVersion(item));
                }
            }
            return serverList;
        }
        ...
        {
            var c = regex.Match(name.ToLower()).Value;
            return c == "" ? "Other" : c;
        }
    }
